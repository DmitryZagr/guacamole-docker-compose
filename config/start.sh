...
associate_openid() {
  set_property "openid-authorization-endpoint" "$KEYCLOAK_HOSTNAME"'/realms/'"$KEYCLOAK_REALM"'/protocol/openid-connect/auth'
  set_property "openid-jwks-endpoint" "$KEYCLOAK_HOSTNAME"'/realms/'"$KEYCLOAK_REALM"'/protocol/openid-connect/certs'
  set_property "openid-issuer" "$KEYCLOAK_HOSTNAME"'/realms/'"$KEYCLOAK_REALM"
  set_property "openid-client-id" "$KEYCLOAK_CLIENT"
  set_property "openid-redirect-uri" "$KEYCLOAK_HOSTNAME"'/realms/'"$KEYCLOAK_REALM"'/protocol/openid-connect/auth?client_id='"$KEYCLOAK_CLIENT"'&redirect_uri='"$GUACAMOLE_HOSTNAME"'&response_mode=fragment&response_type=code&scope=openid'
